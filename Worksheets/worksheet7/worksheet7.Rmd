...
fun <- function(x) { #creating a function named fun that takes in an input x and squares it
  x*x
}
...
nested_reg_lang_hw_cor <- nested_reg_lang |>
  mutate(
    cor_home_work = map_dbl(
      data,
      ~ cor(.x$home, .x$work)
    )
  )

nested_reg_lang_hw_cor
```
