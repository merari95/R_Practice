...
```{r tags=c()}
flavours_and_vessls <- left_join(flavour, vessel) #i am left joining flavour dataframe with vessel dataframe. this means that all the entries in the 1st dataframe are kept. Both dataframes are using the common "name" column
...
airports2 <- airports |> # i am creating the subdataset airports2 from airports 
  select(faa, lat, lon) #i am only selecting these 3 columns

flight_dests <- left_join(flights, airports2, join_by(dest == faa)) #i am left joining flights dataframe with airports2 dataframe and joining by the common column (dest for flights and faa for airports2)

head(flight_dests) #this is a preview of the combined dataframe where all the entries of the left dataframe, flights, are kept
```
...
core_flavours_and_vessls #now i am inner joining core_teaching_team_members with flavours_and_vessls so only common values remain 
```
...
full_core_flavours_and_vessls #this is a full join between core_teaching_team_members and  flavours_and_vessls dataframes, keeping all rows for each dataframe
```
...
```{r tags=c()}
for (a in lyrics) { #here i am creating a 'for' loop. this effectively prints out each item in the lyrics vector
  print(a)
}
...
#here i creating if/else statements that tell the user the amount of reps one must do depending on what is assigned to exercise
