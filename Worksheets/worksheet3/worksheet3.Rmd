...
answer_1 <- mdy(d1) #Here I am using the {lubridate} functions to parse the dates into a standardized format (YYYY-MM-DD). The functions mdy(), ymd(), and dmy() detect the order of month, day, and year automatically and convert the strings into proper date objects that R can recognize.
answer_2 <- ymd(d2)
answer_3 <- dmy(d3)
answer_4 <- mdy(d4)
answer_5 <- mdy(d5)
...
days_of_week <- wday(dates, label = TRUE , abbr = FALSE ) #here i am using the lubridate function wday to get the day of the week from the corresponding dates, the argument label = TRUE allows the date to be in string format and abbr = FALSE prevents the abbreviation of the day
...
```{r tags=c()}
mds_2024_2025 <- interval(start = "2024-09-01", end = "2025-06-30") #here i am creating a time interval with the interval function, the arguments are start (start date) and end (end date)
mds_2024_2025
```
...
A couple hints: - the `holiday_date` column should be converted to the class "Date" - `%within%` is useful for pairing with `filter` to get rows in an interval

```{r tags=c()}

canada <- read_csv("data/canada_2024-25.csv", skip = 2) #here i am reading the canada_2024-25.csv file and skipping the first 2 rows because it doesnt contain any data. the variable contains a dataset of Canada's holidays
canada

mds_stat_holidays_2024_25 <- canada |> #i am calling the same canada variable
  mutate(holiday_date = ymd(holiday_date)) |> #i am mutating the holiday_date column but parsing it to proper date format
  filter(holiday_date %within% mds_2024_2025) #i am only keeping the holidays that fall in the mds interval
...
ottawa_region <- region_lang |> #here i am calling the region_lang
  filter(str_detect(region, "Ottawa")) #i am filtering for rows that have "Ottawa" in their region column, using str_detect function

ottawa_region #leaving only rows with region Ottawa
```
...
replaced_dash <- region_lang |> # i am calling the region_lang dataset
  mutate(region = str_replace(region, "-", "&")) #here i am replacing hypens with ampersands in the region column
...
airports_factor <- airports |>  #im using airports dataset
  mutate(tzone = as_factor(tzone)) #convert the tzone column into a factor. a factor are categories with assigned levels


tzone_levels<-levels(airports_factor$tzone) #i am getting the levels from the tzone column

head(airports_factor) #shows the first 6 rows of the dataset
tzone_levels #prints the tzone_levels
```
